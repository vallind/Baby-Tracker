...
相关测试或 Kotlin 编译，Pull Request 由 CI 完成全量测试、Lint 和 APK 构建。默认创建
Ready PR 并启用 GitHub 原生 Squash Auto-merge；CI 与分支保护条件全部满足后自动合并，
失败时保留 PR 等待修复。Draft PR 只用于明确要求暂不合并的工作。模拟器验证仅在明确
需要 UI 或真机流程验收时执行。
