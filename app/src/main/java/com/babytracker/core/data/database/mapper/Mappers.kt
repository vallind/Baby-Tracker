package com.babytracker.core.data.database.mapper

import com.babytracker.core.database.entity.*
import com.babytracker.core.domain.model.*

/**
 * Room Entity ↔ Domain Model 双向映射。
 *
 * 当前阶段为占位实现，未被 Repository 调用。
 * 后续 Repository 接口切换为 Domain Model 时启用。
 */
...
fun FeedingEntity.toDomain() = Feeding(
    ...
    breastSide = BreastSide.fromRaw(breastSide),
    foodName = foodName,
    amountG = amountG,
    brand = brand,
    note = note,
...
fun Feeding.toEntity() = FeedingEntity(
    id = id,
    babyId = babyId,
    type = FeedingType.raw(type),
    amountMl = amountMl,
    durationMin = durationMin,
    breastSide = BreastSide.raw(breastSide),
    foodName = foodName,
    amountG = amountG,
    brand = brand,
    note = note,
...
fun Vaccination.toEntity() = VaccinationEntity(
    ...
    scheduledDate = scheduledDate,
    administeredDate = administeredDate,
    status = VaccinationStatus.raw(status),
    note = note,
)
