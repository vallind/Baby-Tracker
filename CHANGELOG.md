...
- 设置占位项（隐私设置/帮助与反馈）去 Toast，改灰态行 + 「敬请期待」标签
- 家庭页空态文案迁移 AppStrings（创建/加入双按钮已达标，仅补 i18n）
- AI 聊天（H4）：顶栏可折叠——展开=摘要+模型选择器，收起=一枚胶囊
  （👶 名字 + 模型名，点击展开），聊天区可用空间恢复
- 主题选择器：主题卡预览由单色块升级为「主→次→第三色」渐变条
- 版本号 2.1.6 → 2.1.7（versionCode 42 → 43）
...
- 新增 `RecordDetailSheet` 接入 6 处（喂养/睡眠/尿布/生长/时间线/健康）：卡片单击=详情弹层
  （只读字段 + 编辑主按钮 + 删除次按钮，删除走确认后 Snackbar 撤销，与左滑路径一致），
  长按行为保留；`grep "onClick = {}"` feature 层归零（验收标准 2 达成）
- 睡眠页夜间大卡可点击进入详情；时间线详情编辑按类型分发到 5 个现有表单
- 健康页（T3/C5）：7 分类徽章除疫苗（琥珀）外统一珊瑚分区浅底，去彩虹色；
  `›` 文本箭头全部换 ChevronRight 矢量图标（展开时旋转 90°）
- 版本号 2.1.0 → 2.1.1（versionCode 36 → 37）
...
**交互优化批次二：列表页日期切换增强：**
- 喂养/尿布/睡眠/生长四页日期行增加前一天/后一天箭头按钮，跨天查看免开级联选择器
- 非今天时显示"今天"快捷回跳 chip；点日期文本仍可打开精确日期选择器

### [1.10.0] — 2026-08-23

**交互优化批次一：首页快捷记录 + 表单减负：**
- 首页宫格改造：喂养/睡眠/尿布三格点击直达记录表单（原路径"进列表页→点按钮→弹表单"5 步缩为 1 步），保存后留在首页并自动刷新今日概览，Snackbar 支持撤销（撤销即删除该条）
- 宫格信息架构微调："统计分析"让位（底部导航已有统计 Tab）；"提醒中心"进宫格（原先只有设置页一个深入口）
- 三表单新增时间快捷条（刚刚/15分钟前/30分钟前/1小时前），高频回填免开级联滚轮，精确调整仍走原选择器
- 喂养表单奶量（60/90/120/180ml）与饮水量（50/100/150/200ml）常用值一键填
- 新增 `QuickTimeChipRow` 共享件（datetimecascade 包内共置，按两次规则评估为布局组合、不注册组件令牌）
